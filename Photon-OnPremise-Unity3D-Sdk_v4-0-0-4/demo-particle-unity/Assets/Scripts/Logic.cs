...
public class Logic 
{
    ...
    /// <summary>
    /// Start the game using given connection parameters
    /// </summary>
    ...
    public void StartGame(string serverAddress, string appId, string gameVersion)
    {
        ...
        localPlayer = new GameLogic(serverAddress, appId, gameVersion);
    ...
    {
        GameLogic addedClient = new GameLogic(ServerAddress, AppId, GameVersion);
        addedClient.SetUseInterestGroups(localPlayer.UseInterestGroups);
        addedClient.Start();

        GameObject playerCube = GameObject.CreatePrimitive(PrimitiveType.Cube);
        playerCube.name = addedClient.PlayerName;
        cubes.Add(playerCube);

        backgroundGames.Add(addedClient.PlayerName, addedClient);
    }
